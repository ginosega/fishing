...
PR47 merged as `f05b7ec532d8e45f3fc36c1e0976c5ea40eb9983`; PR CI #239 and production #240 completed successfully, including Pages deployment. The user verified the Gear refinement and Cylinder Weights replacement image. FISH-TODO-058/059 are DONE. The old release handoff is historical. KB Add/Edit is in progress on PR48 / FISH-TODO-060 and must not be described as deployed until verified.
...
| 2026-09-07 | KB authoring | Reuse Gear's validated, copyable handoff workflow, while retaining the KB's complete Markdown document and source/derived picture distinction. | APPROVED / IN PROGRESS / FISH-TODO-060. No direct browser GitHub writes or competing local database. Existing IDs and Markdown paths stay immutable; new IDs use Type prefixes only as a convention. |
| 2026-09-07 | KB picture editing | Keep canonical source records distinct from the transformed display index. Use exact media ownership and shared materialization rules. Preserve original source/provenance in history on replacement/removal; never delete image bytes as a side effect. | APPROVED / IN PROGRESS. Validate complete candidate and transformed bundle, plus stale source/Markdown/media checks. |
| 2026-09-07 | KB Type changes | Reclassify existing entries without changing their stable IDs or document paths. Reject changes that invalidate exact Catch relationships; do not infer replacements. | APPROVED / IN PROGRESS. New IDs use the initial Type prefix; existing prefixes are not validation constraints. |
...
Before new-chat transfer, reconcile the actual repository and outstanding transactions/actions, not merely the remembered conversation. Keep production, approved pending design, and historical state distinct. Preserve all current user-authored files. A feature is completed only after required tests, merge, and actual deployment evidence exist. The September 6 release record is historical. The bootstrap and TODO must point to the current PR48 / FISH-TODO-060 until its production release is verified.
