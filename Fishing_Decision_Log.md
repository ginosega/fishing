...
GitHub upload-folder links generated by authoring now point to the physical source under `pwa/...`, while package and record paths remain logical `Gear/...` / `KB/...` paths.
...
FISH096–FISH103 do not change this decision.
...
- logical frame paths use `KB/Knots/assets/<knot-id>/step-01...`; physical files now live under `pwa/KB/Knots/assets/<knot-id>/`;
...
FISH102 verified production source was `4912f93149e9de1e9cde9ff5176b4a4831a67812`, release `beff9138c96489abd9723c5fcfeef0ff`, [run 34770966552](https://github.com/ginosega/fishing/actions/runs/34770966552), hosted-verification artifact `10322381375`. Canonical counts are **69 Gear, 56 KB and 5 Catches**.

## FISH103 — physical PWA source and authoring fixes

**Decision/status:** DONE and production-verified. Feature [PR125](https://github.com/ginosega/fishing/pull/125) implemented the application/source changes. Verifier-only [PR126](https://github.com/ginosega/fishing/pull/126) and [PR127](https://github.com/ginosega/fishing/pull/127) corrected hosted-test dialog sequencing only; they did not change application behavior, canonical data or release content.
...
- new-KB Markdown Preview must assign a provisional required `content` path before whole-library validation;
- the production workflow no longer needs separate root `Gear/**`, `KB/**` and `Catches/**` path triggers because canonical domain source is within `pwa/**`.

Final verified production source is `94772e62788fa98903930e4b5649fabb9629c6d0`, release `b8c8222697222f1dd43861427d5006fb`, [run 34795289032](https://github.com/ginosega/fishing/actions/runs/34795289032), **327 hosted v2 files**, hosted-verification artifact `10329313590`. The exact-current-main run passed full source/core, Chromium/WebKit preview, production-browser, archived-v1 cutover, exact-main, Pages, hosted-byte and hosted-browser verification.
...
Fishing Companion v3 remains **DEFERRED**. Its future scope includes authentication, direct GitHub save/upload, integrated uploads, offline authoring, outbox/sync, Catch authoring and multi-user generalization. None of those capabilities are implemented or implicitly authorized by FISH096–FISH103.
...
FISH071–076 and FISH078–103 are complete; Fishing Companion v3 (`FISH-TODO-077/P2`) remains deferred. The canonical TODO preserves active fishing/gear/content research and purchase uncertainty. The next unused canonical application task ID is **FISH-TODO-104** unless actual current `main` has already allocated it.