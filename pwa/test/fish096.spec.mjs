...
async function prepare(){
 ...
 run(path.join(pwa,'tools/build.mjs'),['--base='+prefix,'--source='+source,'--out='+out,'--source-revision=fish096-browser']);run(path.join(pwa,'tools/verify.mjs'),['--out='+out,'--source='+source]);pointer=await readJson(path.join(out,'release.json'));
...
test('FISH096 runtime shows final frame, opens frame 1, steps/loops, and remains sequence-capable offline',async({page,context})=>{
 await openReady(page);await route(page,'#/kb/'+sequenceId,'');
 ...
 await disconnect(context,true);await page.reload();await expect(page.locator('#app .page-header h1')).toBeVisible();await route(page,'#/kb/'+sequenceId,'');await page.locator('.picture-button').click();await expect(page.getByRole('dialog',{name:'Image sequence viewer'}).locator('.viewer-frame-indicator')).toHaveText('1 of 3');await page.getByRole('dialog',{name:'Image sequence viewer'}).getByRole('button',{name:'Next'}).click();await expect(page.getByRole('dialog',{name:'Image sequence viewer'}).locator('.viewer-frame-indicator')).toHaveText('2 of 3');await disconnect(context,false);
});
...
test('FISH096 Edit converts a sequence to a new static picture',async({page})=>{
 await openReady(page);await route(page,'#/kb/edit/'+sequenceId,'Edit');const form=page.locator('.editor-form'),action=form.getByRole('combobox',{name:'Picture action'});await action.selectOption('replace-with-static');await form.getByLabel('Choose a local picture').setInputFiles(staticFile);const pkg=await packageFrom(page);expect(pkg.picture.action).toBe('replace');expect(pkg.pictureSequence.action).toBe('remove');expect(pkg.picture.path).toMatch(/KB\/Knots\/assets\/fish096-static\.png$/);
});

test('FISH096 Edit converts a static Knot picture to a complete sequence',async({page})=>{
 await openReady(page);await route(page,'#/kb/edit/'+staticId,'Edit');const form=page.locator('.editor-form'),action=form.getByRole('combobox',{name:'Picture action'});await action.selectOption('replace-with-sequence');const chooser=form.getByLabel('Choose local pictures');await chooser.setInputFiles(sequenceFiles);expect(await chooser.evaluate(input=>input.files.length)).toBe(3);const pkg=await packageFrom(page);expect(pkg.picture.action).toBe('replace');expect(pkg.pictureSequence.action).toBe('set');expect(pkg.pictureSequence.paths).toHaveLength(3);expect(pkg.pictureSequence.paths.every(x=>x.includes(`KB/Knots/assets/${staticId}/`))).toBeTruthy();expect(pkg.picture.path).toBe(pkg.pictureSequence.paths.at(-1));
});
