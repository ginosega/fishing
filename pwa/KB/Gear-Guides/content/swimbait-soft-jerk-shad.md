A lure designed to
imitate a real fish’s swimming motion. Unlike crankbaits or jerkbaits which
depend on erratic action, swimbaits rely on a lifelike swim that fools fish
into thinking they’re chasing real prey. Used to trigger strikes from bigger
predators like bass, pike, or trout that are feeding on baitfish. May get less
last bites, but bigger fish.

## Use

- When targeting big fish -
  swimbaits appeal to large, mature fish that prefer a single, substantial
  meal.
- In clear water - the natural
  profile and motion look ultra-realistic
- Around baitfish
  schools        - shad, bluegill,
  or trout forage
- Post-spawn & in the fall,
  when bass are feeding heavily and hunting larger prey

## Gear

- Rod: 7’–8’ heavy or
  extra-heavy power for big baits, lighter for swimbaits. Moderate to fast
  action.
- Reel: 6.2:1 or slower
  baitcaster with strong drag
- Line: 15–25 lb fluorocarbon
  for most; 65 lb braid for heavy cover
- Leader (optional):
  Fluorocarbon for stealth if using braided mainline
- Hook: 2/0 – 5/0, EWG worm
  offset hook, twistlock centering pin hooks. Video: [The Right Way to Rig a Soft Jerkbait](https://www.youtube.com/watch?v=dOgX9l18DQk)
- Knot: Tie directly to jighead
  or hook when using soft plastics, and to split ring or eye when using
  hard. Okay to use a small snap when swapping hard swimbaits, but not a
  snap-swivel (too heavy)

## Technique

- Target points, ledges, weed
  edges, docks, and around submerged structure
- Retrieve slow and steady -
  the tail or joints provide the motion
- Adjust retrieve speed or
  weight to reach desired depth
- For glide baits, use
  half-turn pauses or gentle twitches to trigger followers

## Bait

|  |  |  |  |  |  |
| --- | --- | --- | --- | --- | --- |
| Type of Swimbait | Description | Typical Size | Hook Setup | Best Use | Example Product(s) |
| Soft Plastic Swimbait | Made of soft rubber with paddle-tail or twin-tail. Includes craws, paddletails, flukes (minnows) and worms. Swims naturally. | 3″–7″ | Rigged on jighead with skirt, belly-hook, or dedicated swimbait hook. Use a lifelike jighead that matches the lure. Use a 1/0 - 2/0 offset hook for 3 and 4" lures, and a 3/0 - 4/0 hook for 5." Heavier jigheads will sink more and can be moved faster through the strike zone. | Versatile lakes, rivers, and ponds | Deps Sakamata Shad  Strike King Tour Grade Swim Jig and Rage Swimmer,  Berkley Power Jerk Shad,  6th Sense Divine Swimbait,  Yamamoto Hinge |
| Hard Jointed Swimbait | Multi-segment hard body (plastic or wood) with realistic “S-swim” | 5″–12″+ | Built-in treble hooks, sometimes line-through | Trophy fish, clear water, big bites | Shimano Armajoint 190SS |
| Glide Bait | Single joint or multi-segment with wide side-to-side “glide” action | 6″–12″+ | Treble hooks, often large profile | Big bass, open water, reaction strikes | Bass Pro Shops XPS Swerve Glide Swimbait |
| Line-Through Swimbait | Designed so the line passes through the body; hook separates when fish bites | 5″–9″ | Hidden treble hook, line-through system | Big predators, heavy cover, tricky fish | Big Bite Baits B6 Line‑Thru Swimbait |


## Links

- My Gear: [Fin-Sanity Bluegill](gear://fin-sanity-bluegill), [6th Sense Divine Swimbait](gear://sixth-sense-divine-swimbait), [Berkley Power Jerk Shad](gear://berkley-powerbait-power-jerk-shad), [VMC Swimbait Jig](gear://vmc-swimbait-jig), [Owner Twistlock Centering Pin Hook](gear://owner-twistlock-black-chrome-centering-pin-hook), [Gamakatsu EWG Worm Offset Hook](gear://gamakatsu-ewg-worm-offset-hook)
- KB: [Snaps & Swivels](kb://technique-snaps-swivels)
