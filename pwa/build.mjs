...
for (const [src, dest] of kbFiles) {
  const markdown = await fs.readFile(path.join(repoRoot, src), 'utf8');
...
const youtubeCorpus = [...kbMarkdown, JSON.stringify(gearSeed)];
...
await fs.writeFile(path.join(out, 'build.json'), JSON.stringify({
  ...
  source: 'Structured IndexedDB-backed My Gear + GitHub Markdown knowledge base',
