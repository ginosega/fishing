...
import { kbPictureSource } from './kb-picture-model.js';
...
// Preserve canonical picture metadata for authoring, but package only active
// hero assets. Authored Markdown references are copied independently below.
const kbMediaConfig = JSON.parse(await fs.readFile(path.join(here, 'local-media.json'), 'utf8'));
const kbMarkdown = [];
...
for (const entity of kbSeed.entities) {
  ...
  if (entity.picture?.src && !kbPictureSource(entity.id, kbMediaConfig) && !/^https?:\/\//i.test(entity.picture.src)) {
...
const mediaItems = (mediaManifest.items || []).filter(item => mediaOwners.has(item.id))
...
await fs.writeFile(path.join(out, 'build.json'), JSON.stringify({
  buildVersion, builtAt:new Date().toISOString(),
  source:'Structured IndexedDB-backed My Gear + unified Markdown-content Knowledge Base + structured Catch Log',
  gearSchemaVersion:gearSeed.schemaVersion, gearRecords:gearSeed.items.length,
  kbSchemaVersion:kbSeed.schemaVersion, kbEntities:kbSeed.entities.length,
  catchSchemaVersion:catchSeed.schemaVersion, catches:catchSeed.catches.length,
  kbOfflineAssets:kbAssets.size, gearImages:successfulMedia.length,
  requestedGearImages:mediaItems.length, videoTitles:Object.keys(videoTitles).length
}, null, 2));
...
async function buildGearMedia(item) {
  try {
    ...
    return { id:item.id, owners:item.owners, asset:`./assets/gear/${filename}`, alt:item.alt || item.id,
      destination:item.destination || item.sourcePage || '', sourcePage:item.sourcePage || '',
      imageSource:imageUrl, sourceKind:'remote', bytes:bytes.length };
  } catch (error) {
