...
async function renderEntity(id) {
  ...
  const catches = catchesForEntity(state.catches, catchField(entity.type), entity.id);
  app.innerHTML = `${pageHeader(entity.name, entity.description || '', `#/kb/${plural(entity.type)}`)}
    ...
    ${catchField(entity.type) ? catchBacklinks(catches) : ''}`;
  bindRoutes();
}
...
function catchField(type) { return ({ location:'locationId', species:'speciesId', technique:'techniqueId' })[type] || ''; }
