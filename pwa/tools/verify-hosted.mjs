...
try{
 ...
 if(production){
  ...
  const icon=await page.evaluate(async()=>await(await fetch('./manifest.webmanifest')).json());assert.equal(icon.icons[0].src,'./revised-icon.png');
