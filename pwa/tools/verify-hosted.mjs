...
try{
 ...
 assert.deepEqual(await page.evaluate(()=>window.__FISHING_V2__.counts),{gear:80,kb:56,catches:5});for(const dir of ['Gear','KB','Catches']){await fs.access(dir);await assert.rejects(fs.access(path.join('..',dir)),error=>error?.code==='ENOENT');}
