...
const manifest=JSON.parse(await fs.readFile(path.join(args.dist,pointer.manifest),'utf8'));const expectedCounts=manifest.counts;report.counts=expectedCounts;
...
try{
 ...
 assert.deepEqual(await page.evaluate(()=>window.__FISHING_V2__.counts),expectedCounts);for(const dir of ['Gear','KB','Catches']){await fs.access(dir);await assert.rejects(fs.access(path.join('..',dir)),error=>error?.code==='ENOENT');}
 ...
 if(production){
  ...
  await page.goto(root+'#/catches');await page.getByRole('heading',{name:'Recorded catches',exact:true}).waitFor();assert.equal(await page.locator('.record-grid h2 + time').count(),expectedCounts.catches);assert.equal(await page.locator('.catch-date').first().evaluate(e=>getComputedStyle(e).fontWeight),'400');await page.getByRole('button',{name:'Back',exact:true}).waitFor();
  ...
  page.once('dialog',dialog=>dialog.accept());await page.goto(root+'#/inventory/edit/daiwa-tatula-xt-rod');const caption=page.locator('.editor-form').getByLabel('Caption (optional)',{exact:true});await caption.click();await caption.pressSequentially('Hosted caption focus');assert.equal(await caption.evaluate(el=>document.activeElement===el),true);assert.equal(await caption.inputValue(),'Hosted caption focus');
