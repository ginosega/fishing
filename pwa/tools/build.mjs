...
export async function build(){
 ...
 try{
  ...
  const releaseId=digest(Buffer.from(stableJson({sourceRevision,base,pendingMedia,files:inputHashes,icon:digest(icon),app:digest(app.outputFiles[0].contents),css:digest(css),lock:digest(lock),code:codeHashes}))).slice(0,32);
