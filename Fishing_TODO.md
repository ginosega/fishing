...
| FISH-TODO-038 | P1 | WAITING ON USER | PWA / GitHub Pages | Enable GitHub Pages for the Fishing repo with Source = GitHub Actions. | Build and Pages artifact upload already succeed; first deploy returned 404 because the Pages site is not enabled. One-time action: repository Settings → Pages → Build and deployment → Source: GitHub Actions. Then rerun the failed deploy/build workflow. |
