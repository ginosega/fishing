...
| FISH-TODO-034 | P3 | OPEN | Markdown usability | Spot-check inline links in GitHub Preview. | Applies to authored documents under `pwa/kb-content/` and migrated/reference Markdown where GitHub Preview remains useful. |
...
| FISH-TODO-046 | 2026-09-02 | PWA / Knowledge Base architecture | Adopted and implemented one unified KB Entity schema for Location, Species, Technique, and Knot: ID, Type, Name, optional Description, optional Picture, and one complete Markdown Content document. Planner/session/trip-history concepts were retired. Released in PR #13, merge commit `1c61cb0b001bb9f543904a2328178632ac0efed1`; production run #77 / `33644412492` succeeded. |
| FISH-TODO-047 | 2026-09-02 | PWA / Catch Log | Migrated five historical catches to structured records with stable species, location, and lure IDs; optional setup/technique fields remain null where not historically recorded. Catch backlinks now use exact IDs rather than text matching. Released and production-verified with PR #13/run #77. |
| FISH-TODO-049 | 2026-09-02 | Project handoff / operating mode | Reconciled authoritative context, decisions, TODOs, PWA records, KB design status, and the new-chat bootstrap after the unified KB release. Added the durable instruction that Fishing uses Chat mode by default and requires a specific Work-only need plus user approval before recommending a temporary switch. |
