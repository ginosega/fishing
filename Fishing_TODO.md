...
| FISH-TODO-052 | P2 | IN PROGRESS | Fishing Companion acceptance / Markdown | Finish PR #28 Equipment/Technique Markdown formatting cleanup and final acceptance pass. | User found formatting errors in many PR #28-created/refreshed documents and is correcting the files directly in GitHub under `pwa/kb-content/techniques/`. Do not rename/move files without updating `kb.seed.json`. After edits, run normal validation/build/deploy and spot-check representative Equipment/Technique pages, images, links, and Search/filter placement. |
...
| FISH-TODO-054 | 2026-09-04 | Project recovery / handoff | Reconstructed the interrupted-chat build sequence from repository history and recovered prior context. No distinct post-PR #29 feature branch/commit/PR or recoverable unmerged application change was found. Current resume point is PR #28 authored-content acceptance/formatting cleanup, then the canonical open backlog. |
