...
| FISH-TODO-084 | P1 | DONE | All eight UI/authoring refinements live in PR89; direct hosted checks passed. Automated verifier selector follow-up is FISH085. Exact uploaded icon, origin-aware post-copy Exit, prefixed JSON, Catch dates, search wording, KB/Catch labels and arrow-free Back buttons. KB Notes remain required. See the current closeout above and approved-scope record. |
| FISH-TODO-085 | P1 | WAITING ON USER | Permanent hosted-verifier correction: narrow Yellow Perch page-heading selector to level 1, then complete automated hosted verification. All runtime tests and deployment passed; direct browser follow-up verified live behavior. A follow-up PR/release would exceed the user’s explicit one-PR/one-release instruction, so obtain direction before that additional repository work. |
