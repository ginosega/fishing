## FISH091 production-complete continuation point — September 11, 2026

FISH091 is **DONE**. Requirements PR103 and design PR104 were separately approved before implementation. [PR105](https://github.com/ginosega/fishing/pull/105) implemented the approved online-only default and explicit offline-library flow. Current verified production source is `44fa3bcbff289d2a7aa37c4967e0f5f195b53ca0`, release `01c5ed3535d358086032b0985e2af87b`, [run 34676802369](https://github.com/ginosega/fishing/actions/runs/34676802369), with 214 hosted v2 files.

On normal online use, do **not** expect or trigger a whole-library download. Online full loads use current production. **Connection Status → Update offline library** is the explicit full-library preparation/refresh action. A complete older generation may remain as offline fallback until explicitly refreshed; failed/corrupt updates preserve the prior generation. A device without a prepared complete generation must not claim offline readiness. FISH077/P2 remains **DEFERRED**. Next unused canonical task ID is **FISH-TODO-096**.

## FISH095 current continuation point — September 11, 2026

Current verified production source is `44fa3bcbff289d2a7aa37c4967e0f5f195b53ca0`, release `01c5ed3535d358086032b0985e2af87b`, [run 34676802369](https://github.com/ginosega/fishing/actions/runs/34676802369), with 214 hosted v2 files byte-verified and hosted browser verification passed.
...
FISH090/[PR95](https://github.com/ginosega/fishing/pull/95) is DONE. Edit Gear can add the first link when an existing record omits `links`. Current canonical `pwa/icon.png` is the exact user-uploaded 1,541,464-byte, 1254×1254 RGBA replacement (SHA-256 `055cea4acde10ac18a74cc0a42f03a03190ea4a12df17acc0d5808a1637b822a`); temporary `icon-v3.png` is removed. Production source `470c74472e569010b81659c86fdd60937d6a9159` passed run `34634890084`.

Historical snapshot — FISH091/[draft PR96](https://github.com/ginosega/fishing/pull/96) is **requirements-only, WAITING ON USER and not implemented**. It proposes changing the automatic complete-offline-library behavior to an online-only default with explicit preparation via Connection Status → Update offline library. Do not treat the draft as authorization to change runtime, service worker, loader, UI, build or tests. Required sequence: user requirements signoff → separate abbreviated design → user design signoff → implementation/release.

FISH092/[PR97](https://github.com/ginosega/fishing/pull/97) is DONE at source `ef883688c3f0786e35821f0baca5394c8f2156a5`, run `34640020315`: every KB category durably has `content/` and `assets/`, with `KB/README.md` and structural regression coverage. Existing approved Gear-Guide reuse of canonical Gear-owned images remains valid. FISH093/[PR98](https://github.com/ginosega/fishing/pull/98) is DONE at source `347beab04ce60bb372c390bd8820b25c3d7b6314`, run `34642839586` attempt 2: every Gear category durably has `assets/` and `content/`, with `Gear/README.md` and structural regression coverage. These are repository-layout decisions, not record/schema changes.
...
FISH-TODO-089 is DONE through [PR94](https://github.com/ginosega/fishing/pull/94). Verified production source is `5f413021f51a7d48ea8f7f96e99d9a81db9ee0a6`, release `7a7f62759f668cda0eece08125e798b2`, with successful [production run 34625693090](https://github.com/ginosega/fishing/actions/runs/34625693090). The full source/core, preview-browser, production-browser, archived-v1, exact-current-main Pages deployment and hosted byte/browser gates passed; the hosted verifier reports 209 v2 files.

PR94 applied four validated KB location-picture packages: Cranberry Lake now uses `KB/Locations/assets/Cranberry Lake.jpg`; Lake Sammamish uses `KB/Locations/assets/Lake Sammamish.jpg`; Mayfield Lake uses `KB/Locations/assets/Mayfield Lake.png` and the submitted muskie/Ike Kinswa description; Silver Lake uses `KB/Locations/assets/Silver Lake.jpg`. Captions are the submitted dated captions. All four Notes/Markdown files were kept. The four files were pre-existing newer user uploads on restored `main` and matched the submitted sizes/SHA-256 values exactly. The old Cranberry PNG remains because deletion was not requested. No schema, architecture or P2 scope changed.

FISH084–089 are DONE. FISH077/P2 remains DEFERRED. All unrelated backlog and purchase uncertainty remain preserved. The next unused canonical task ID is FISH-TODO-090. Resume from actual latest `main` and the user's next requested work; do not restart completed FISH089 release work. This documentation reconciliation does not redeploy production.

## FISH088 Kingforest inline-spinner production closeout — September 11, 2026

FISH-TODO-088 is DONE through [PR93](https://github.com/ginosega/fishing/pull/93). Verified production source is `364a599a4eeb51457ece46457d738ebb4f0d82f3`, release `97e6a320ed42c432b875de9fe8c2ef25`, with successful [production run 34621210887](https://github.com/ginosega/fishing/actions/runs/34621210887). The full source/core, preview-browser, production-browser, archived-v1, exact-current-main Pages deployment and hosted byte/browser gates passed; the hosted verifier reports 206 v2 files.

PR93 applied the validated Gear package for `generic-1-inline-spinner`: current name **Kingforest #1 inline spinner**, manufacturer **Kingforest**, empty structured links, supplied Mayfield Lake/Amazon/internal-technique Notes and picture `Gear/Lures/assets/Kingforest Inline Spinner.png` captioned **Kingforest #1 Inline Spinner**. The image was an existing newer user upload on restored `main` and exactly matched the package (430,419 bytes; SHA-256 `57416d3c9d6dcee3e4fabfc324bd281756be5c9ff5cfc2b5a41affbbaaec8b14`). Regression expectations were updated only for the resulting 235 → 236 referenced-source count and elimination of the last pictureless Lure; absent-picture behavior is still tested on a KB Location. No schema, architecture or P2 scope changed.

FISH084–088 are DONE. FISH077/P2 remains DEFERRED. The historical migration decision that the generic inline-spinner image was optional remains evidence only; the current Gear record now has the user-supplied image. All unrelated backlog and purchase uncertainty remain preserved. The next unused canonical task ID is FISH-TODO-089. Resume from actual latest `main` and the user's next requested work; do not restart completed FISH088 release work. This documentation reconciliation does not redeploy production.

## FISH087 KB authoring batch production closeout — September 11, 2026

FISH-TODO-087 is DONE through [PR92](https://github.com/ginosega/fishing/pull/92). Verified production source is `dee0ff76f6b15e861fa8864151aad88c8fe28f5e`, release `40c849b4014013f22200843c9d222c2c`, with successful [production run 34614884273](https://github.com/ginosega/fishing/actions/runs/34614884273). The run passed complete source/core validation, preview- and production-scope browser acceptance, archived-v1 cutover acceptance, exact-current-main Pages deployment, all 205 hosted-file comparisons and hosted browser verification.

PR92 applied the validated KB location/species change package. The unrelated newer Silver Lake Markdown edit was preserved; the user-restored `KB/Locations/content/lake-bosworth.md` is present in the deployed release. The only non-content correction was a regression assertion updated to expect no Species subtitle after intentional description removal. No schema, architecture, picture, direct-Save/P2 or broader authoring decision changed.

FISH084–087 are DONE. FISH077/P2 remains DEFERRED. All unrelated fishing/equipment/content backlog and purchase uncertainty remain preserved. The next unused canonical task ID is FISH-TODO-088. Resume from actual latest main and the user's next requested work; do not restart completed release work. This documentation reconciliation does not redeploy production.
...
The source-equivalence waiver stands; no separate physical-device/IndexedDB export or inspection is claimed. Retain old browser stores and reconcile later-discovered local-only data before retirement. FISH076, FISH078 and FISH083 are DONE; P2 FISH077 remains DEFERRED. FISH084–090 and FISH092–095 are DONE and fully verified in production. FISH091 is DONE and production-verified through PR105, source `44fa3bcbff289d2a7aa37c4967e0f5f195b53ca0`, release `01c5ed3535d358086032b0985e2af87b`, run `34676802369`. All originally required missing pictures are resolved; the historically optional generic inline-spinner has the user-supplied Kingforest picture through FISH088, FISH089 adds the four validated location pictures, FISH094 associates the validated Trout Fishing picture without changing its Notes, and FISH095 adds the validated Topwater Fishing and Bass Fishing Techniques with exact `TODO` Notes. Unrelated backlog remains; next unused task ID is FISH-TODO-096. Update Context, Decision Log, TODO, README, affected source/technical/release records and this bootstrap after actual milestones; cross-check before handoff.