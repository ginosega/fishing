...
test('the complete source inventory hashes and fully decodes all referenced assets',async()=>{
 ...
 assert.equal(result.data.gear.items.length,69);assert.equal(result.images.size,81);assert.equal(result.pending.length,6);assert.equal(result.references.length,235);
