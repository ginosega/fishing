...
try{
 ...
 if(production){assert.equal(await page.locator('#release-details').count(),0);assert.equal(pointer.pendingMedia,false);await page.goto(root+'#/kb/species-perch');await page.getByRole('heading',{name:'Yellow Perch',exact:true}).waitFor();assert.equal(await page.locator('.picture-empty').count(),0);await page.goto(root+'#/catches');await page.getByRole('heading',{name:'Catch Log',exact:true}).waitFor();assert.equal(await page.locator('.record-grid .nav-card').count(),5);await page.screenshot({path:'hosted-evidence/production-catches.png',fullPage:true});}else{await page.goto(root);await page.getByRole('button',{name:/My Gear Browse/}).waitFor();}report.browser=production?'production navigation, counts, image viewer, complete offline reload, absent pictures and no release diagnostics passed':'v1 navigation, preview with existing root worker, image viewer, complete offline reload, return to v1 passed';
 await context.close();
