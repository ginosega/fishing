...
const source=path.resolve(option('--source',repo)),output=path.resolve(option('--out',path.join(repo,'v2/dist')));
...
export async function build(){
 ...
 try{
  ...
  const app=await esbuild.build({entryPoints:[path.join(repo,'v2/src/entry.mjs')],bundle:true,format:'iife',platform:'browser',target:'es2022',minify:true,write:false,legalComments:'none',define:{'process.env.NODE_ENV':'"production"'}});
  const css=await fs.readFile(path.join(repo,'v2/src/styles.css'));
  const inputHashes=await hashFiles(content);const lock=await fs.readFile(path.join(repo,'v2/package-lock.json'));
  const codeHashes=[];for(const dir of ['src','tools'])for(const file of await hashFiles(path.join(repo,'v2',dir)))codeHashes.push({...file,path:dir+'/'+file.path});
  ...
  const loader=await fs.readFile(path.join(repo,'v2/src/loader.js'));
  const index=`<!doctype html>\n<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="theme-color" content="#11665c"><meta name="description" content="Fishing reference and catch log"><link rel="manifest" href="./manifest.webmanifest"><link rel="icon" href="./icon.svg" type="image/svg+xml"><title>Fishing Companion</title><script defer src="./loader.js"></script></head><body><div class="preview-banner"${pendingMedia?'':' hidden'}>V2 preview · Pending media · Production remains unchanged</div><header class="site-header"><div class="header-inner"><a class="brand" data-nav href="#/">Fishing <span>Companion</span></a><nav class="site-nav" aria-label="Main navigation"><a data-nav href="#/inventory">My Gear</a><a data-nav href="#/kb">Knowledge Base</a><a data-nav href="#/catches">Catch Log</a></nav></div></header><main id="app" class="site-main" tabindex="-1"><p>Loading Fishing Companion…</p></main><aside id="app-notice" class="site-main" aria-live="polite" style="min-height:0;padding-top:0;padding-bottom:0"></aside><footer class="site-footer"><div class="footer-inner"><div class="offline-panel"><span id="offline-status" class="offline-state" role="status">Offline incomplete</span><button id="offline-update" class="btn secondary" type="button">Update offline library</button><button id="offline-reload" class="btn secondary" type="button">Reload</button></div><span id="release-details"></span></div></footer></body></html>\n`;
  ...
  const sw=await esbuild.build({entryPoints:[path.join(repo,'v2/src/sw.mjs')],bundle:true,format:'iife',platform:'browser',target:'es2022',minify:true,write:false,define:{__RELEASE_ID__:JSON.stringify(releaseId)}});
