...
class Migration:
 def __init__(self,args):
  self.a=args;self.source=Path(args.source);self.archive=Path(args.archive);self.out=Path(args.output)
 ...
 def copy(self,src,dest,expected=None):
  ...
  if not any(r['path']==dest for r in self.report['files']):self.report['files'].append({'path':dest,'bytes':len(data),'sha256':h,'source':rel or str(src)})
  return dest
