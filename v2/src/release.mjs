...
export function validateManifest(manifest,pointer,base){
 ...
 const required=['index.html','loader.js','sw.js','manifest.webmanifest','icon.svg',`releases/${pointer.id}/app.js`,`releases/${pointer.id}/styles.css`,`releases/${pointer.id}/content/Gear/gear.json`,`releases/${pointer.id}/content/KB/kb.json`,`releases/${pointer.id}/content/Catches/catches.json`];
