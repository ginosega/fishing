...
export function createApp(ctx){
 ...
 function navigate(hash){if(hash===currentHash)return;if(!canLeave())return;location.hash=hash;}
 ...
 async function render(){const ticket=++serial;disposeEditor();route=parseRoute(currentHash);root.replaceChildren(el('div',{class:'loading'},'Loading…'));
  ...
  }catch(error){if(ticket===serial)root.replaceChildren(missing('Unable to display this page: '+error.message));console.error(error);}
 }
