...
const ROOT_SUBTITLES={gear:'Browse your inventory of equipment, tackle, and bait',kb:'Fishing reference and catch log'};
...
export function createApp(ctx){
 ...
 function listControls(domain,records,{search=false,filter=false,sort='name'}={}){
  ...
  if(search){const searchInput=input('',{type:'search',placeholder:'Search '+(domain==='gear'?'gear':'Knowledge Base')});searchInput.addEventListener('input',()=>{query=searchInput.value.toLocaleLowerCase();draw();});searchInput.setAttribute('aria-label','Search');controls.append(searchInput);}
 ...
 function renderRoot(domain){const gear=domain==='gear',rows=gear?GEAR_CATEGORIES:KB_TYPES,all=ctx.data[domain][DOMAIN[domain].array];const page=frame(gear?'My Gear':'Knowledge Base','#/',[],ROOT_SUBTITLES[domain]);
  const searchInput=input('',{type:'search',placeholder:'Search all '+(gear?'gear':'Knowledge Base')});const searchResults=el('div',{class:'record-grid',hidden:true});const categories=el('div',{class:'nav-grid'},...rows.map(([key,label])=>{const meta=CATEGORY_PRESENTATION[domain][key];const item=card(label,meta.description||'',categoryRoute(domain,key),meta.icon);if(meta.svg)item.querySelector('.card-icon').innerHTML=meta.svg;return item;}));
  ...
  searchInput.setAttribute('aria-label','Search all '+(gear?'gear':'Knowledge Base'));page.querySelector('.page-actions').prepend(searchInput);page.append(categories,searchResults,anchor(gear?'Add Gear':'Add Entry',gear?'#/inventory/add/lures':'#/kb/add/technique','page-edit-link'));return page;
 }
 function renderList(domain,key){const gear=domain==='gear',rows=gear?GEAR_CATEGORIES:KB_TYPES;const definition=rows.find(x=>x[0]===key);if(!definition)return missing('Unknown category',rootRoute(domain));
  ...
  const list=listControls(domain,records,{search:gear?key==='lures':key==='equipment',filter:gear,sort:gear?'gear':'name'});page.querySelector('.page-actions').prepend(...list.controls.children);page.append(list.element,anchor(gear?'Add Gear':'Add Entry',gear?`#/inventory/add/${key}`:`#/kb/add/${key}`,'page-edit-link'));return page;
 }
 ...
 async function renderEditor(domain,id,category,type){const record=id?ctx.maps[domain].get(id):null;if(id&&!record)return missing('This item does not exist or its ID has been retired.',rootRoute(domain));const parent=record?routeFor(domain,record.id):categoryRoute(domain,domain==='gear'?category:type);
  const page=frame(record?'Edit '+record.name:domain==='gear'?'Add Gear':'Add Entry',parent,record?[button('Create Link',()=>copyLink(domain,record.id))]:[]);const created=await createEditor(ctx,{domain,baseRecord:record,category,type,onBack:()=>navigate(parent),onDirty:()=>{}});editor=created;page.append(created.element);return page;
 }
 ...
 const updateConnection=()=>{connection.dataset.online=String(navigator.onLine);document.getElementById('connection-network').textContent=navigator.onLine?'Online':'Offline';};
 ...
 document.getElementById('offline-update').disabled=false;
