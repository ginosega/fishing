...
export function createOffline({root,releaseId,onStatus,isDirty,notify}){
 ...
 async function update(){
  if(!registration){await start();if(!registration)return;}
  ...
  }catch(error){await command('STATUS').catch(()=>{});publish({...status,message:error.message});}
 }
 function reload(){if(isDirty()&&!confirm('Your prepared changes have not been saved. Discard the current edit and reload?'))return;location.reload();}
